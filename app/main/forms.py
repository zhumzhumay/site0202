...
class EditProfileForm(FlaskForm):
    ...
    kkal = DecimalField(label='Базовая потребность в ККал', widget=widgets.NumberInput(max=1500, min=50, step=1),
                        validators=[DataRequired()])
    BG = DecimalField(label='Целевой уровень гликемии, ммоль/л', widget=widgets.NumberInput(max=20, min=1, step=0.1),
                      validators=[DataRequired()])
    carb = DecimalField(label='Критический уровень углеводов, г', widget=widgets.NumberInput(max=500, min=1, step=0.1),
                        validators=[DataRequired()])
...
class FoodForm(FlaskForm):
    ...
    # food2 = SearchField()
    eating = SelectField(u'Прием пищи', choices=[('1', 'Ужин'), ('2', 'Обед'), ('3', 'Завтрак'), ('4', 'Перекус')],
                         validators=[DataRequired()])
    time = DateTimeLocalField(label='Время', format='%Y-%m-%dT%H:%M')
    grams = IntegerField(label='вес, г', default=200, widget=widgets.NumberInput(min=1, max=600, step=1),
                         validators=[DataRequired()])
...
class InsulinForm(FlaskForm):
    ...
    dose = IntegerField(label='ед.', default=5, widget=widgets.NumberInput(min=1, max=40, step=1),
                        validators=[DataRequired()])
